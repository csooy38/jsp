...
public class BbsDAO {
	...
	public List<BbsDTO> getBbsList(int page, int rowsize) {
		...
		try {
			openConn();
			sql = "select * from (select row_number() over(order by board_group desc, board_step) rnum, b.* from jsp_bbs b) where rnum >= ? and rnum <= ?";
			pstmt = con.prepareStatement(sql);
	...
	// jsp_bbs 테이블의 게시물 조회수를 증가시키는 메서드
	public void bbsHit(int bbs_no) {
	...
	// jsp_bbs 테이블에서 중간에 삭제된 글이 있는 경우 글 번호를 다시 정비하는 메서드
	public void updateNo(int bbs_no) {
	...
	public void replyUpdate(int bbs_group, int bbs_step) {
