...
<body>
	...
	<div align="center">
		...
		<table border="1" cellspacing="0" width="400">
			<%
				if(dto != null) { // 검색된 레코드가 있는 경우 웹 브라우저에 출력
			%>
				<tr>
					<th>제품번호</th>
					<td><%=dto.getPnum() %></td>
				...
				<tr>
					<td colspan="2" align="center">
						...
						<input type="button" value="제품삭제" 
							onclick="if(confirm('삭제하시겠습니까?')) {location.href='delete.do?num=<%=dto.getPnum() %>'
									}else {return;}">
						<%-- 자바스크립트에서 window 객체 하위의 confirm() 메서드는 확인/취소 버튼을 가진 경고창을 만든다.
							확인 버튼을 클릭하면 반환값으로 true를 리턴하고, 취소 버튼을 클릭하면 반환값으로 false를 리턴한다.
							즉, 삭제 유무를 다시 한 번 확인하기 위해서 주로 사용된다.
							확인 버튼을 클릭하면 delete.do라는 서블릿으로 이동을 하고, 
							취소 버튼을 클릭하면 삭제를 하지 않고 그대로 현재 창에 있게 되는 코드 --%>
					</td>
				</tr>
		</table>
	</div>
