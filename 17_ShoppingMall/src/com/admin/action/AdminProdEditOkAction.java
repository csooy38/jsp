...
public class AdminProdEditOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 상품 수정 폼 페이지에서 넘어 온 데이터들을 DB에 저장하는 컨트롤러 클래스
		...
		if(p_image == null) {	// 새로운 첨부파일 이미지가 없는 경우
			p_image = multi.getFilesystemName("p_image_old");
		}
		...
		int check = dao.updateProdect(dto);
