...
public class UploadUpdateOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 자료실 수정 폼 페이지에서 넘어 온 데이터들을 DB에 저장하는 클래스. 
		
		UploadDTO dto = new UploadDTO();
		
		// 첨부파일이 저장될 경로(위치) 설정
		String saveFolder = "C:\\NCS\\transfer\\15_Board_FileUpload\\WebContent\\upload";
		...
		MultipartRequest multi = new MultipartRequest(
				...
				"UTF-8",							// 문자 인코딩 방식
				new DefaultFileRenamePolicy());		// 파일의 이름이 중복되지 않게 설정
		...
		// type="file"로 넘어 온 데이터는 getFile() 메서드로 받아주어야 한다.
		...
		if(upload_file != null) {	// 첨부파일이 있는 경우
			...
			if(!path1.exists()) {	// 해당 폴더가 존재하지 않는 경우
				path1.mkdir();	// 실제로 폴더가 만들어 짐
			}
			
			// "작성자_파일명" 형식으로 파일을 만들어 보자.
